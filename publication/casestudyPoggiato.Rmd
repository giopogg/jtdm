...
In this R markdown we reproduce the case study of the publication Poggiato et al., In preparation,  "Joint models and predictions of community traits". In this code we use a previous version of the R package \code{jtdm}, whose functions have been coded in the R script 'JTDM_functions.R'. See https://giopogg.github.io/jtdm/ for a full description of the R package. In this previous version we relied on JAGS to sample from the posterior distribution
...
# Source JTDM functions
source("JTDM_functions.R")
```

Load the dataset and run the model

```{r, warning=FALSE,message=FALSE,results='hide'}
load("X.rda") #env variables
load("Y.rda") #site x CWM traits matrix

formula=as.formula("~poly(GDD,2)+poly(FDD,2)+poly(GDD,2):forest+poly(FDD,2):forest")


# Run the model. We reduced the number of samples to ensure a fast compilation of the vignette. To obtain the results of the publication, we set adapt = 5000, burnin = 10000, sample = 10000.
m = jtdm_fit(Y=Y, X=X, 
               formula=formula,
               adapt = 500, burnin = 1000, sample = 1000)
```
...
CV = jtdmCV(m, K=5, adapt = 500, burnin = 1000, sample = 1000)
